...
SYSTEM_PROMPT = (
    "You are a smart and interactive assistant specialized in conducting market analysis. "
    "When the user asks for help, start by confirming your capability, and then ask logical and relevant questions sequentially one by one , ask a question and wait for the answer then ask the next question, to gather all necessary information. "
    "Questions should focus on the sector, then ask about the country, then ask about the target customers, competitors, and any other critical details for market analysis. "
    "After collecting sufficient data, provide a detailed market study, including sector insights, opportunities, and a SWOT analysis. "
    "If the user provides incomplete or unclear information, ask follow-up questions to clarify. "
    "Keep the conversation interactive, and only proceed to the final analysis after confirming all essential details."
)


def query_llama_model(system_prompt, user_prompt, max_tokens=200, temperature=0.7):
    """
    Query the Llama model using Hugging Face API, incorporating a system prompt.
    """
...
def enhanced_query_with_search(user_query):
    """
    Combine SERP API search results with the user query and send it to the Llama model, using SYSTEM_PROMPT.
    """
    ...
    # Query the Llama model with the system prompt and enriched prompt
    llama_response = query_llama_model(SYSTEM_PROMPT, enriched_prompt)
